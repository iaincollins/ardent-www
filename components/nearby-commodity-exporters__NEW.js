...
// TODO Refactor to optimise for distance / volume / landing pad size / date 
// with fallbacks to broadend the search if there are only a few results
async function getNearbyExportersOfCommodity(systemAddress, commodityName) {
  const url = `${API_BASE_URL}/v2/system/address/${systemAddress}/commodity/name/${commodityName}/nearby/exports?maxDistance=25&fleetCarriers=false&sort=distance&minVolume=1000`
  const res = await fetch(url)
  const commodities = await res.json()

  if (commodities.length < 10) {
    const url = `${API_BASE_URL}/v2/system/address/${systemAddress}/commodity/name/${commodityName}/nearby/exports?maxDistance=100&sort=distance`
    const res = await fetch(url)
    commodities.concat(await res.json())
  }
