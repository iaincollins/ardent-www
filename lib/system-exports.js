...
module.exports = async (systemAddress) => {
  const allCommodities = await getCommoditiesWithPricing()
  const res = await fetch(`${API_BASE_URL}/v2/system/address/${systemAddress}/commodities/exports?fleetCarriers=0`)
  ...
  exportOrders.forEach(c => {
    ...
    if (!exportOrdersGroupedByCommodity[symbol]) {
      exportOrdersGroupedByCommodity[symbol] = {
        ...
        category: (allCommodities.find(el => el.symbol.toLowerCase() === symbol))?.category ?? 'Unknown',
