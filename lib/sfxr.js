...
const params_signed = ['p_freq_ramp', 'p_freq_dramp', 'p_arp_mod', 'p_duty_ramp', 'p_pha_offset', 'p_pha_ramp', 'p_lpf_ramp', 'p_hpf_ramp']
...
Params.prototype.toB58 = function () {
  ...
  return (function (B, A) { const d = []; let s = ''; let i; let j; let c; let n; for (i in B) { j = 0, c = B[i]; s += c || s.length ^ i ? '' : 1; while (j in d || c) { n = d[j]; n = n ? n * 256 + c : c; c = n / 58 | 0; d[j] = n % 58; j++ } } while (j--)s += A[d[j]]; return s }(convert, b58alphabet))
}
...
sfxr.b58decode = function (b58encoded) {
  ...
  for (const pi in params_order) {
    ...
    if (p == 'wave_type') {
      ...
      const val = (decoded[offset] | (decoded[offset + 1] << 8) | (decoded[offset + 2] << 16) | (decoded[offset + 3] << 24))
...
SoundEffect.prototype.initForRepeat = function () {
  ...
  if (ps.p_arp_mod >= 0) { this.arpeggioMultiplier = 1 - Math.pow(ps.p_arp_mod, 2) * 0.9 } else { this.arpeggioMultiplier = 1 + Math.pow(ps.p_arp_mod, 2) * 10 }
...
var _sfxr_getAudioFn = function (wave) {
  return function () {
    ...
    if (actx) {
      const buff = actx.createBuffer(1, wave.buffer.length, wave.header.sampleRate)
      const nowBuffering = buff.getChannelData(0)
...
const domain = {
  ...
  p_freq_dramp: function (v) { return (v * 44100 / Math.pow(2, -44101.0 / 44100)) },

  p_vib_speed: function (v) { return (441000 / 64.0 * v) },
  p_vib_strength: function (v) { return (v * 100) },

  p_arp_mod: function (v) { return (1.0 / v) },
  p_arp_speed: function (v) { return (v / 44100) },

  p_duty: function (v) { return (100 * v) },
  p_duty_ramp: function (v) { return (8 * 44100 * v) },

  p_repeat_speed: function (v) { return v === 0 ? 0 : (44100.0 / v) },

  p_pha_offset: function (v) { return (1000 * v / 44100) },
  p_pha_ramp: function (v) { return (1000 * v) },

  p_lpf_freq: function (v) { return (v === 0.1) ? 0 : 8 * 44100 * v / (1 - v) },
  p_lpf_ramp: function (v) { return Math.pow(v, 44100) },
  p_lpf_resonance: function (v) { return (100 * (1 - v * 0.11)) },
...
const domain_inverse = {
  ...
  p_freq_dramp: function (v) { return v * Math.pow(2, -44101.0 / 44100) / 44100 },

  p_vib_speed: function (v) { return (64.0 / 441000) * v },
  p_vib_strength: function (v) { return (v / 100) },
  ...
  p_lpf_resonance: function (v) { return (1 - v / 100) / 0.11 },
...
const units = {
  ...
  p_freq_dramp: function (v) {
    return (v * 44100 / Math.pow(2, -44101.0 / 44100)).toExponential(4) +
      ' 8va/s^2'
  },
  ...
  p_vib_speed: function (v) {
    return v === 0
      ? 'OFF'
      : (441000 / 64.0 * v).toPrecision(4) + ' Hz'
  },
